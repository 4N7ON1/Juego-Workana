...
CMapData::CMapData(class CGame * pGame)
{
	...
	for (i = 1; i <= 6; i++)
	{
		...
		m_stFrame[i][DEF_OBJECTGETITEM].m_sFrameTime = 50 / 1.6;
	...
	// MarketMob
	m_stFrame[130][DEF_OBJECTSTOP].m_sFrameTime = 200;
	m_stFrame[130][DEF_OBJECTSTOP].m_sMaxFrame = 3;
	m_stFrame[130][DEF_OBJECTMOVE].m_sFrameTime = 90;
	m_stFrame[130][DEF_OBJECTMOVE].m_sMaxFrame = 3;
	m_stFrame[130][DEF_OBJECTATTACK].m_sFrameTime = 80;
	m_stFrame[130][DEF_OBJECTATTACK].m_sMaxFrame = 3;
	m_stFrame[130][DEF_OBJECTDAMAGE].m_sFrameTime = 60;
	m_stFrame[130][DEF_OBJECTDAMAGE].m_sMaxFrame = 3;
	m_stFrame[130][DEF_OBJECTDYING].m_sFrameTime = 65;
	m_stFrame[130][DEF_OBJECTDYING].m_sMaxFrame = 3 + 3;

	// MarketMob
	m_stFrame[131][DEF_OBJECTSTOP].m_sFrameTime = 200;
	m_stFrame[131][DEF_OBJECTSTOP].m_sMaxFrame = 3;
	m_stFrame[131][DEF_OBJECTMOVE].m_sFrameTime = 90;
	m_stFrame[131][DEF_OBJECTMOVE].m_sMaxFrame = 3;
	m_stFrame[131][DEF_OBJECTATTACK].m_sFrameTime = 80;
	m_stFrame[131][DEF_OBJECTATTACK].m_sMaxFrame = 3;
	m_stFrame[131][DEF_OBJECTDAMAGE].m_sFrameTime = 60;
	m_stFrame[131][DEF_OBJECTDAMAGE].m_sMaxFrame = 3;
	m_stFrame[131][DEF_OBJECTDYING].m_sFrameTime = 65;
	m_stFrame[131][DEF_OBJECTDYING].m_sMaxFrame = 3 + 3;

	// new
	m_stFrame[132][DEF_OBJECTSTOP].m_sFrameTime = 250;
	m_stFrame[132][DEF_OBJECTSTOP].m_sMaxFrame = 7;
	m_stFrame[132][DEF_OBJECTMOVE].m_sFrameTime = 40;
	m_stFrame[132][DEF_OBJECTMOVE].m_sMaxFrame = 4;
	m_stFrame[132][DEF_OBJECTATTACK].m_sFrameTime = 60;
	m_stFrame[132][DEF_OBJECTATTACK].m_sMaxFrame = 12;
	m_stFrame[132][DEF_OBJECTDAMAGE].m_sFrameTime = 120;
	m_stFrame[132][DEF_OBJECTDAMAGE].m_sMaxFrame = 7;
	m_stFrame[132][DEF_OBJECTDYING].m_sFrameTime = 70;
	m_stFrame[132][DEF_OBJECTDYING].m_sMaxFrame = 7 + 3;


	// new
	m_stFrame[133][DEF_OBJECTSTOP].m_sFrameTime = 250;
	m_stFrame[133][DEF_OBJECTSTOP].m_sMaxFrame = 7;
	m_stFrame[133][DEF_OBJECTMOVE].m_sFrameTime = 40;
	m_stFrame[133][DEF_OBJECTMOVE].m_sMaxFrame = 4;
	m_stFrame[133][DEF_OBJECTATTACK].m_sFrameTime = 60;
	m_stFrame[133][DEF_OBJECTATTACK].m_sMaxFrame = 12;
	m_stFrame[133][DEF_OBJECTDAMAGE].m_sFrameTime = 120;
	m_stFrame[133][DEF_OBJECTDAMAGE].m_sMaxFrame = 7;
	m_stFrame[133][DEF_OBJECTDYING].m_sFrameTime = 70;
	m_stFrame[133][DEF_OBJECTDYING].m_sMaxFrame = 7 + 3;
		...
		for (dY = 0; dY < MAPDATASIZEY; dY++)
		{
			...
			if (m_pData[dX][dY].m_sOwnerType != NULL)
			{
				...
				if ((m_pData[dX][dY].m_iStatus & 0x40000) != 0) // haste
					iDelay -= (m_stFrame[m_pData[dX][dY].m_sOwnerType][DEF_OBJECTRUN].m_sFrameTime) / 3.0;
					...
					if (m_pData[dX][dY].m_cOwnerAction == DEF_OBJECTSTOP) { // DEF_OBJECTSTOP = 1 // 00497334
						...
						case 6: // glowing armor/weapon
							if ((m_pData[dX][dY].m_cOwnerFrame == 1) || (m_pData[dX][dY].m_cOwnerFrame == 5))
							{
								...
								//if ((m_pData[dX][dY].m_iStatus & 0x10) == 0 && m_pGame->bPlayerShines && !m_pGame->isInMap("battle") ) m_pGame->bAddNewEffect(54, (m_sPivotX + dX) * 32 + (rand() % 20 - 10), (m_sPivotY + dY) * 32 - (rand() % 50) - 5, NULL, NULL, -(rand() % 8), 0);
/*
								...
								/*if ((((m_pData[dX][dY].m_iStatus & 0x00000F00) >> 8) > rand() % 6) // Angel stars
									...
									&& ((m_pData[dX][dY].m_iStatus & 0x10) == 0) && !m_pGame->isInMap("battle"))
								{
						...
						case 54: // Dark-Elf
							...
							if ((m_pData[dX][dY].m_cOwnerFrame == 1 && m_pData[dX][dY].m_bSpriteOmit) || (m_pData[dX][dY].m_cOwnerFrame == 5 && m_pData[dX][dY].m_bSpriteOmit)) {
								...
								{*/
									...
									//if ((m_pData[dX][dY].m_iStatus & 0x10) == 0 && m_pGame->bPlayerShines && !m_pGame->isInMap("battle") ) m_pGame->bAddNewEffect(54, (m_sPivotX + dX) * 32 + dx + (rand() % 20 - 10), (m_sPivotY + dY) * 32 + dy - (rand() % 50) - 5, NULL, NULL, -(rand() % 8), 0);
					...
					if ((m_pData[dX][dY].m_cOwnerAction == DEF_OBJECTATTACK)) { //3 00498685
						...
						case 6: // Humans
							...
							if (m_pData[dX][dY].m_cOwnerFrame == 5)
							{
								...
								/*	bool noenter = false;
									auto specItem = m_pGame->get_special_item(m_pData[dX][dY].m_wObjectID);
									if (specItem != -1)
									{
										int sx = m_sPivotX + dX;
										int sy = m_sPivotY + dY;

										int tx = sx + m_pData[dX][dY].m_sV1;
										int ty = sy + m_pData[dX][dY].m_sV2;
										short ownerType = m_pData[dX][dY].m_sOwnerType;

										if (specItem == 1) //120 fireball
										{									//120		
											/ *m_pGame->bAddNewEffect(120, sx, sy, tx, ty, 0, ownerType);* /
											noenter = true;
										}
										else if (specItem == 2) //120 lighting
										{
											m_pGame->bAddNewEffect(143, sx, sy, tx, ty, 0, ownerType);											
											noenter = true;
										}
										else if (specItem == 3) //120 ls
										{
											m_pGame->bAddNewEffect(174, sx, sy, tx, ty, 0, ownerType);										
											noenter = true;
										}
										else if (specItem == 4) //120 child wind
										{
											m_pGame->bAddNewEffect(145, sx, sy, tx, ty, 0, ownerType);											
											noenter = true;
										}
										else if (specItem == 5) //120 tremor
										{
											m_pGame->bAddNewEffect(138, sx, sy, tx, ty, 0, ownerType);
											noenter = true;
										}
										else if (specItem == 6) //120 thor
										{
											m_pGame->bAddNewEffect(186, sx, sy, tx, ty, 0, ownerType);
											noenter = true;
										}
