...
int XSocket::_iOnRead()
{
	...
	if (m_cStatus == DEF_XSOCKSTATUS_READINGHEADER)
	{
		...
		{
			// 🔹 Detectar half-close (sin desconectar)
			char testByte;
			int test = recv(m_Sock, &testByte, 1, MSG_PEEK);

			if (test > 0)
			{
				// Hay datos pendientes → seguir leyendo
				return DEF_XSOCKEVENT_ONREAD;
			}
			else if (test == 0)
			{
			...
			else
			{
				...
				{
					// 🔸 Half-close detectado (sigue vivo)
					return DEF_XSOCKEVENT_HALFCLOSE;
				}
				else
				{
					m_cType = DEF_XSOCK_SHUTDOWNEDSOCK;
