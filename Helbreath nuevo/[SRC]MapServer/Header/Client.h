...
public:
	...
	uint32 m_runTurn;
	bool running;
	int GetCurrentStatLevel(int statIndex) const;

	bool SpecialCharacter();
	
	//void ClearBag(int client);
	void ClearBag();
	void ClearDrops();
	...
	int m_iCharacterRegen;
	int m_iCharacterRegenTime;
	int m_iCharacterRegenMode;

	int m_iCharacterQuestx2;
	int m_iCharacterQuestx3;
	int m_iCharacterGoodRate;
	int m_iRegenEndu;
	int m_iRegenEnduTime;
	...
	bool m_bAntiPull;
	int m_iAntiPullCount;
	BOOL  m_bIsMsgSendAvailable;
	BOOL  m_bIsCheckingWhisperPlayer;

	//heaton fake limbo
	bool m_bForceDisconnect;
	char m_cMacAdrees[18];
	bool m_bLimbo;
	bool m_bHP_Neck;
	short m_sForceTeleportShop;

	int lastAttackerID;
	DWORD lastDamageTime;
	DWORD m_dwLastKeepAlive; // guarda el último timestamp (en ms)

	//HelBot
	BotState botState;
	bool m_bHelBot;
	bool m_bAltDebuffStep;
	bool m_bBeingCalled;
	bool m_bReachedGuardPoint;
	short fakemovedir;
	short weapontype;
	short m_sCallX;
	short m_sCallY;
	int m_iPots;
	int m_iAssignedGuardPoint;
	int m_iTargetPlayer;
	int m_iTargetX;
	int m_iTargetY;
	int currentTarget;   
	DWORD m_dwLastKnockbackTime;
	DWORD lastActionTime;
	DWORD m_dwLastMoveTime;
	DWORD m_dwLastAttackTime;
	DWORD m_dwMoveDelay;    
	DWORD m_dwAttackDelay; 
	DWORD m_dwTargetSearchTime; 
	DWORD m_dwTargetSearchDelay;
	DWORD m_dwLastHealTime;
	DWORD m_dwLastDebuffTime;
	std::map<char, DWORD> m_mBlockedDirections;
	//
	...
	short m_sFireStatus;
	char m_cFireIndex[12];
	short m_sFireTime;

	short m_sLightingStatus;
	char m_cLightingIndex[12];
	short m_sLightingTime;
	int spec_phys_damage;
	int spec_magic_damage;
	// Player
	...
	class CItem * m_pSaveItemList[DEF_MAXITEMS];
	class CItem * m_pGungameItems[DEF_MAXITEMS];
	POINT m_ItemGungamePosList[DEF_MAXITEMS];
	...
	BOOL  m_bIsGGItemEquipped[DEF_MAXITEMS];
	short m_sItemEquipmentStatus[DEF_MAXITEMEQUIPPOS];
	char  m_cArrowIndex;

	char           m_cBackMagicMastery[DEF_MAXMAGICTYPE];
	...
	int   m_iMaxAP_SM;
	int   m_iMaxAP_L;
	bool m_bSpecFeatures;
	bool m_bVipFeatures;
	bool m_bSXFeat;
	bool m_bInArenaLoby;
	BOOL  m_bIsNeutral;
	BOOL  m_bIsObserverMode;
	unsigned long lastSpecialEagleCharacterExecutionTime = 0; // Tiempo de la última ejecución
	unsigned long lastSpecialCharacterExecutionTime = 0; // Tiempo de la última ejecución
	unsigned long lastHPNeckExecutionTime = 0; // Tiempo de la última ejecución
	unsigned long lastNassaSpecialCharacterExecutionTime = 0; // Tiempo de la última ejecución
	unsigned long lastBossCharacterExecutionTime = 0; // Tiempo de la última ejecución
	unsigned long lastAKsCharacterExecutionTime = 0; // Tiempo de la última ejecución
	unsigned long lastNewRegenCharacterExecutionTime = 0; // Tiempo de la última ejecución
	unsigned long lastDavidCharacterExecutionTime = 0; // Tiempo de la última ejecución
	unsigned long lastVipCharacterExecutionTime = 0; // Tiempo de la última ejecución
	unsigned long lastRyanCharacterExecutionTime = 0; // Tiempo de la última ejecución
	unsigned long lastSpecCharacterExecutionTime = 0; // Tiempo de la última ejecución
	unsigned long lastVipReqVit = 0; // Tiempo de la última ejecución
	unsigned long lastRegenEndurance = 0; // Tiempo de la última ejecución

	unsigned long lastRegenLevel1ExecTime = 0; // Tiempo de la última ejecución
	unsigned long lastRegenLevel2ExecTime = 0; // Tiempo de la última ejecución
	unsigned long lastRegenLevel3ExecTime = 0; // Tiempo de la última ejecución
	unsigned long lastRegenLevel4ExecTime = 0; // Tiempo de la última ejecución
	unsigned long lastRegenLevel5ExecTime = 0; // Tiempo de la última ejecución
	unsigned long lastFishingAction = 0; // Tiempo de la última ejecución
	unsigned long lastReqFishingAction = 0; // Tiempo de la última ejecución
...
//	int GetItemLastIndex(short ItemID); // Retorna el indice en el arreglo de items del ultimo con ese ID. Error: -1
	...
	bool IsRelicHolder()
	{
		return m_bRelicHolder;
	}
	//timer sg
	DWORD dw_LastSG;

	// NPC Kill Counter System - Using NPC names as unique identifiers
	std::map<std::string, int> m_mapNpcKillCount;   // Key: NpcName, Value: Kills actuales
	std::map<std::string, int> m_mapNpcHuntLevel;   // Key: NpcName, Value: Nivel de caza
	void AddNpcKill(int npcType, const char* npcName);
};
