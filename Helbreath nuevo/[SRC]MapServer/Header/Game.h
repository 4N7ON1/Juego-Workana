...
// Definici�n de la estructura para representar un �tem inicial
struct StarterItem {
	std::string itemName;   // Nombre del �tem
	DWORD itemAttr;   // Atributos del �tem
	int itemCount;          // Cantidad del �tem
};
...
struct BattlePlayers {
	...
	int client_index; // �ndice del cliente
...
public:
	...
	// Versi�n ultra-r�pida de trim izquierdo
	inline void fast_ltrim(char*& str) {
		while (*str == ' ' || *str == '\t') str++;
	}

	// Atoi optimizado para nuestro caso espec�fico
	...
	bool m_bAbaddonEventActive;     // Si el evento est� activo
	...
	std::unordered_map<int, DWORD> m_mLastCallTime; // �ltimo tiempo de llamada por facci�n
...
//	void RequestPurchaseItemHandler2(int iClientH, char * pItemName, int iNum); // MORLA 2.4 - Para el trade items
	...
	void NpcKilledHandler(short sAttackerH, char cAttackerType, int iNpcH, short sDamage, BOOL bHeld = TRUE, BOOL bSilentKill = FALSE);
...
//	void GlobalEndTpEvent(short Event);
	...
	void EnableBattleRoyale();                // M�todo para iniciar el evento
	...
	void monitorRoundDurationBattleRoyale();  // Monitorea los 10 minutos de duraci�n de la ronda
	...
	int m_durationSecondsBr;        // Duraci�n del temporizador en segundos (5 minutos)
	int m_elapsedSecondsBr;         // Segundos transcurridos durante la cuenta regresiva
	int m_elapsedRoundSecondsBr;    // Segundos transcurridos desde que comenz� la ronda
	int m_iSafeTimeBr;
	...
	int m_sRoundBr;                 // N�mero de la ronda actual
	...
	std::vector<int> m_coordinateIndicesBr;          // �ndices de las coordenadas disponibles
	int m_currentIndexBr;                            // �ndice actual para asignar coordenadas
	float m_radiusBr;            // Radio actual del c�rculo
	bool m_radiusInitializedBr;  // Bandera para saber si el radio ha sido inicializado
	int m_remainingIntervalsBr;  // N�mero de intervalos restantes para reducir el radio

	// Variables para el freeze time
	int m_freezeTimeBr;          // Duraci�n del freeze time en segundos (5 segundos)
	...
	// Hunter Ranking System Helpers
	const char* GetNpcNameByType(int npcType);
	int GetNpcTypeByName(const char* npcName);
	int GetNpcTier(const char* npcName);
	int GetRequiredKills(int npcType, int level);
	int GetRequiredKillsByName(const char* npcName, int level);
	void SendNpcKillData(int client);
	void SaveNpcKillData(int client);
	void LoadNpcKillData(int client);
	void AddNpcKill(int client, int npcType); // Corrected to be consistent
	void LoadNpcHuntConfig();
	// NPC Hunt Ranking System Configuration
	struct st_npchunt_config {
		int m_iMaxLevel;
		int m_iTier; // [NEW] Tier configurable
		std::vector<int> m_iRequiredKills; // Kills needed for each level (index 0 = level 1, etc.)
	};
	std::map<string, st_npchunt_config> m_mapNpcHuntConfig;
};
