...
public:
	...
	// NPC Kill Counter System
	struct st_npc_kill_data {
		char m_cNpcName[22];
		int m_iKillCount;
		int m_iHuntingLevel;
		int m_iMaxKillsPerLevel;
		int m_iMaxLevel; // [NEW] Max Level from config
		BYTE m_iTier; // Tier del NPC (1-11, 0 = sin tier)
	};
	std::vector<st_npc_kill_data> m_vNpcKillList;
	bool m_bShowNpcKillWindow = false;
	int m_iNpcKillScrollPos = 0;
	int m_iSelectedNpcTier = 1; // 1-11 = Tier específico
	void DrawNpcKillWindow(short msX, short msY, char cLB);
	BOOL ClickNpcKillWindow(short msX, short msY, char cRB = 0);
	void ReqNpcKillData();
	...
	// Movement Smoothing
	float m_fSmoothViewPointX, m_fSmoothViewPointY;
	DWORD m_dwLastSmoothTime;
...
*/
	...
	// NPC Hunter Rankings window drag and drop
	short m_sNpcKillWindowX;
	short m_sNpcKillWindowY;
	BOOL m_bNpcKillWindowDragging;
	short m_sNpcKillWindowDragStartX;
	short m_sNpcKillWindowDragStartY;
	...
	void PutString_Outline(int iX, int iY, char * pString, short sR, short sG, short sB, BOOL bCenter = FALSE);
