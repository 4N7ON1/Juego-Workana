...
void CSprite::PutSpriteFastWidth(int sX, int sY, int sFrame, int sWidth, DWORD dwTime)
{
 ...
 RECT rcRect;
	...
	m_rcBound.right  = dX + szx;
	m_rcBound.bottom = dY + szy;
	// === SFML INTERCEPT (Fase 9.B - PutSpriteFastWidth) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
...
void CSprite::PutAlphaSprite(int sX, int sY, int sFrame, DWORD dwTime, int alphaLevel)
{
	...
	// === SFML INTERCEPT (Fase 8.H - PutAlphaSprite) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
		...
		if (g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			int cx = g_pRenderBackend->GetCropX(), cy = g_pRenderBackend->GetCropY();
			g_pRenderBackend->DrawSpriteColor(dX + cx, dY + cy, sx, sy, szx, szy,
				m_iSpriteIndex, 255, 255, 255, alphaLevel);
			m_bOnCriticalSection = FALSE;
			return;
		}
	}
...
void CSprite::PutTransSprite(int sX, int sY, int sFrame, DWORD dwTime, int alphaDepth)
{
 ...
 WORD * pSrc, * pDst;
	...
	// === SFML INTERCEPT (Fase 8.H - PutTransSprite - 50% alpha) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
...
void CSprite::PutTransSpriteMoreMiniScaled(int sX, int sY, int sFrame, DWORD dwTime, int alphaDepth) {
	...
	// === SFML INTERCEPT (Fase 9.B - PutTransSpriteMoreMiniScaled) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
		...
		if (g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			...
			g_pRenderBackend->DrawSpriteScaled(dX + cx, dY + cy, sx, sy, szx, szy,
				m_iSpriteIndex, 0.40f, 0.40f, 255, 255, 255, sfmlAlpha);
			m_bOnCriticalSection = FALSE;
			return;
		}
	}
...
void CSprite::PutTransSprite_NoColorKey(int sX, int sY, int sFrame, DWORD dwTime, int alphaDepth)
{
 ...
 WORD * pSrc, * pDst;
	...
	// === SFML INTERCEPT (Fase 8.H - PutTransSprite_NoColorKey - 50% alpha) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
...
void CSprite::PutTransSprite70(int sX, int sY, int sFrame, DWORD dwTime)
{
 ...
 WORD * pSrc, * pDst;
	...
	// === SFML INTERCEPT (Fase 8.H - PutTransSprite70 - 70% alpha) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
		...
		if (g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			int cx = g_pRenderBackend->GetCropX(), cy = g_pRenderBackend->GetCropY();
			g_pRenderBackend->DrawSpriteColor(dX + cx, dY + cy, sx, sy, szx, szy,
				m_iSpriteIndex, 255, 255, 255, 179);
			m_bOnCriticalSection = FALSE;
			return;
		}
	}
...
void CSprite::PutTransSprite70_NoColorKey(int sX, int sY, int sFrame, DWORD dwTime)
{
 ...
 WORD * pSrc, * pDst;
	...
	// === SFML INTERCEPT (Fase 8.H - PutTransSprite70_NoColorKey - 70% alpha) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
...
void CSprite::PutTransSprite50(int sX, int sY, int sFrame, DWORD dwTime)
{
 ...
 WORD * pSrc, * pDst;
	...
	// === SFML INTERCEPT (Fase 8.H - PutTransSprite50 - 50% alpha) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
...
void CSprite::PutTransSprite50_NoColorKey(int sX, int sY, int sFrame, DWORD dwTime)
{
 ...
 WORD * pSrc, * pDst;
	...
	m_rcBound.right  = dX + szx;
	m_rcBound.bottom = dY + szy;
	// === SFML INTERCEPT (Fase 9.B - PutTransSprite50_NoColorKey) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
...
void CSprite::PutTransSprite25(int sX, int sY, int sFrame, DWORD dwTime)
{
 ...
 WORD * pSrc, * pDst;
	...
	// === SFML INTERCEPT (Fase 8.H - PutTransSprite25 - 25% alpha) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
...
void CSprite::PutTransSprite25_NoColorKey(int sX, int sY, int sFrame, DWORD dwTime)
{
 ...
 WORD * pSrc, * pDst;
	...
	m_rcBound.right  = dX + szx;
	m_rcBound.bottom = dY + szy;
	// === SFML INTERCEPT (Fase 9.B - PutTransSprite25_NoColorKey) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
...
void CSprite::PutFadeSprite(short sX, short sY, short sFrame, DWORD dwTime)
{
 ...
 //int dX,dY,sx,sy,szx,szy,pvx,pvy,sTmp;
	...
	// === SFML INTERCEPT (Fase 8.H - PutFadeSprite) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
...
void CSprite::PutSpriteRGB(int sX, int sY, int sFrame, int sRed, int sGreen, int sBlue, DWORD dwTime)
{
 ...
 WORD * pSrc, * pDst;
	...
	// === SFML INTERCEPT (Fase 8.H - PutSpriteRGB) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
		...
		if (g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			int cx = g_pRenderBackend->GetCropX(), cy = g_pRenderBackend->GetCropY();
			int sfR = (sRed >= 0) ? 255 : (255 + sRed < 0 ? 0 : 255 + sRed);
			int sfG = (sGreen >= 0) ? 255 : (255 + sGreen < 0 ? 0 : 255 + sGreen);
			int sfB = (sBlue >= 0) ? 255 : (255 + sBlue < 0 ? 0 : 255 + sBlue);
			g_pRenderBackend->DrawSpriteColor(dX + cx, dY + cy, sx, sy, szx, szy,
...
void CSprite::PutSpriteFastNoColorKeyDstGrayscale(LPDIRECTDRAWSURFACE7 lpDstS, int sX, int sY, int sFrame, DWORD dwTime)
{
	...
	// Tiles grayscale van siempre por DDraw -> PDBGS (diseño híbrido)
...
void CSprite::PutTintedSpriteRGB(int sX, int sY, int sFrame, int sRed, int sGreen, int sBlue, DWORD dwTime)
{
	...
	m_rcBound.bottom = dY + szy;
		...
		// === SFML INTERCEPT (Fase 9.B - PutTintedSpriteRGB overload1) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
...
void CSprite::PutTransSpriteRGB(int sX, int sY, int sFrame, int sRed, int sGreen, int sBlue, DWORD dwTime)
{
 ...
 WORD  * pSrc, * pDst;
	...
	// === SFML INTERCEPT (Fase 8.H - PutTransSpriteRGB - alpha 128) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
		...
		if (g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			...
			int sfB = (sBlue >= 0) ? 255 : (255 + sBlue < 0 ? 0 : 255 + sBlue);
			g_pRenderBackend->DrawSpriteColor(dX + cx, dY + cy, sx, sy, szx, szy,
				m_iSpriteIndex, sfR, sfG, sfB, 128);
			m_bOnCriticalSection = FALSE;
			return;
		}
	}
...
void CSprite::PutTransSpriteRGB_NoColorKey(int sX, int sY, int sFrame, int sRed, int sGreen, int sBlue, DWORD dwTime)
{
 ...
 WORD  * pSrc, * pDst;
	...
	// === SFML INTERCEPT (Fase 8.H - PutTransSpriteRGB_NoColorKey - alpha 128) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
...
void CSprite::PutRevTransSprite(int sX, int sY, int sFrame, DWORD dwTime, int alphaDepth)
{
	...
	// === SFML INTERCEPT (Fase 8.H - PutRevTransSprite) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
		...
		if (g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			int cx = g_pRenderBackend->GetCropX(), cy = g_pRenderBackend->GetCropY();
			int sfAlpha = min(255, alphaDepth * 4);
			g_pRenderBackend->DrawSpriteColor(dX + cx, dY + cy, sx, sy, szx, szy,
				m_iSpriteIndex, 255, 255, 255, sfAlpha);
			m_bOnCriticalSection = FALSE;
			return;
		}
	}
