...
BOOL XSocket::bConnect(char* pAddr, int iPort, unsigned int uiMsg)
{
	...
	// ==============================
	// 🔹 MEJORA 1: activar KeepAlive
	// ==============================
	int keepAlive = 1;
	setsockopt(m_Sock, SOL_SOCKET, SO_KEEPALIVE, (char*)&keepAlive, sizeof(keepAlive));

#ifdef TCP_KEEPIDLE
	DWORD keepIdle = 30000;     // 30 segundos inactivo
	DWORD keepInterval = 10000; // cada 10 segundos
	DWORD keepCount = 3;        // 3 intentos
	...
	// ==============================
	// 🔹 MEJORA 2: eliminar delay (TCP_NODELAY)
	// ==============================
	int flag = 1;
	...
	// ==============================
	// 🔹 Dirección y conexión asíncrona
	// ==============================
	memset(&saTemp, 0, sizeof(saTemp));
	saTemp.sin_family = AF_INET;
	saTemp.sin_addr.s_addr = inet_addr(pAddr);
...
#ifdef _DEBUG
	char addr[32] = { 0 };
	if (iGetPeerAddress(addr) == 0)
	{
		char tmp[128];
		sprintf(tmp, "[XSocket] Cerrando conexión con %s", addr);
		OutputDebugStringA(tmp);
	}
#endif

	// ===============================
	// 🔹 Cierre completo y seguro
	// ===============================
	// Importante: cerrar ambos canales (lectura y escritura)
	shutdown(m_Sock, SD_BOTH);

	// Recibir cualquier dato pendiente antes de cerrar
	char cTmp[256];
	while (true)
	{
		int iRet = recv(m_Sock, cTmp, sizeof(cTmp), 0);
		if (iRet <= 0) break; // 0 = cerrado, SOCKET_ERROR = fin
	}
