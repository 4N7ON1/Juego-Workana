...
//		szx = szx - (m_pDDraw->m_rcClipArea.left - dX);
	...
	// === SFML INTERCEPT (Fase 8.D - Sprites con colorkey - Sprite2) ===
	// IsFrameActive(): solo activo en juego (entre BeginFrame/EndFrame).
	// En login/seleccion de personaje -> false -> cae a DDraw BltFast correcto.
	// Esto arregla: personaje erroneo (tiles SFML sobreescribian el personaje DDraw).
...
{short dX,dY,sx,sy,szx,szy,pvx,pvy;
 RECT rcRect;
	...
	m_rcBound.right  = dX + szx;
	m_rcBound.bottom = dY + szy;
	// === SFML INTERCEPT (Fase 9.B - PutSpriteFastNoColorKey - Sprite2) ===
...
void CSprite::PutSpriteFastWidth(int sX, int sY, int sFrame, int sWidth, DWORD dwTime)
{
 ...
 RECT rcRect;
	...
	m_rcBound.right  = dX + szx;
	m_rcBound.bottom = dY + szy
		// === SFML INTERCEPT (Fase 9.B - PutSpriteFastWidth - Sprite2) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
...
void CSprite::PutShadowSprite(int sX, int sY, int sFrame, DWORD dwTime)
{
	...
	// === SFML INTERCEPT (Fase 9.B - PutShadowSprite - Sprite2) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
		...
		if (g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			float destX = (float)(sX + pvx - szy / 3);
			float destY = (float)(sY + pvy + 2 * szy / 3);
			g_pRenderBackend->DrawSpriteShadow(sx, sy, szx, szy, m_iSpriteIndex,
				destX, destY, 1.0f / 3.0f, 1.0f / 3.0f);
			m_bOnCriticalSection = FALSE;
			return;
		}
	}
...
void CSprite::PutShadowSpriteClip(int sX, int sY, int sFrame, DWORD dwTime)
{
 ...
 WORD * pSrc, * pDst;
	...
	// === SFML INTERCEPT (Fase 9.B - PutShadowSpriteClip - Sprite2) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
...
void CSprite::PutTransSprite(int sX, int sY, int sFrame, DWORD dwTime, int alphaDepth)
{
 ...
 WORD * pSrc, * pDst;
	...
	// === SFML INTERCEPT (Fase 9.B - PutTransSprite - Sprite2) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
...
void CSprite::PutTransSprite_NoColorKey(int sX, int sY, int sFrame, DWORD dwTime, int alphaDepth)
{
 ...
 WORD * pSrc, * pDst;
	...
	m_rcBound.right  = dX + szx;
	m_rcBound.bottom = dY + szy;
	// === SFML INTERCEPT (Fase 9.B - PutTransSprite_NoColorKey - Sprite2) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
...
void CSprite::PutTransSprite70(int sX, int sY, int sFrame, DWORD dwTime)
{
 ...
 WORD * pSrc, * pDst;
	...
	m_rcBound.right  = dX + szx;
	m_rcBound.bottom = dY + szy;
	// === SFML INTERCEPT (Fase 9.B - PutTransSprite70 - Sprite2) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
		...
		if (g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			int cx = g_pRenderBackend->GetCropX(), cy = g_pRenderBackend->GetCropY();
			g_pRenderBackend->DrawSpriteColor(dX + cx, dY + cy, sx, sy, szx, szy,
				m_iSpriteIndex, 255, 255, 255, 178);
			m_bOnCriticalSection = FALSE;
			return;
		}
	}
...
void CSprite::PutTransSprite70_NoColorKey(int sX, int sY, int sFrame, DWORD dwTime)
{
 ...
 WORD * pSrc, * pDst;
	...
	m_rcBound.right  = dX + szx;
	m_rcBound.bottom = dY + szy;
	// === SFML INTERCEPT (Fase 9.B - PutTransSprite70_NoColorKey - Sprite2) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
...
void CSprite::PutTransSprite50(int sX, int sY, int sFrame, DWORD dwTime)
{
 ...
 WORD * pSrc, * pDst;
	...
	m_rcBound.right  = dX + szx;
	m_rcBound.bottom = dY + szy;
	// === SFML INTERCEPT (Fase 9.B - PutTransSprite50 - Sprite2) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
...
void CSprite::PutTransSprite50_NoColorKey(int sX, int sY, int sFrame, DWORD dwTime)
{
 ...
 WORD * pSrc, * pDst;
	...
	m_rcBound.right  = dX + szx;
	m_rcBound.bottom = dY + szy;
	// === SFML INTERCEPT (Fase 9.B - PutTransSprite50_NoColorKey - Sprite2) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
...
void CSprite::PutTransSprite25(int sX, int sY, int sFrame, DWORD dwTime)
{
 ...
 WORD * pSrc, * pDst;
	...
	m_rcBound.right  = dX + szx;
	m_rcBound.bottom = dY + szy;
	// === SFML INTERCEPT (Fase 9.B - PutTransSprite25 - Sprite2) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
...
void CSprite::PutTransSprite25_NoColorKey(int sX, int sY, int sFrame, DWORD dwTime)
{
 ...
 WORD * pSrc, * pDst;
	...
	m_rcBound.right  = dX + szx;
	m_rcBound.bottom = dY + szy;
	// === SFML INTERCEPT (Fase 9.B - PutTransSprite25_NoColorKey - Sprite2) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
...
void CSprite::PutTransSprite2(int sX, int sY, int sFrame, DWORD dwTime)
{
 ...
 WORD * pSrc, * pDst;
	...
	m_rcBound.right  = dX + szx;
	m_rcBound.bottom = dY + szy;
	// === SFML INTERCEPT (Fase 9.B - PutTransSprite2 - Sprite2) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
...
void CSprite::PutShiftTransSprite2(int sX, int sY, int shX, int shY, int sFrame, DWORD dwTime)
{
 ...
 WORD * pSrc, * pDst;
	...
	m_rcBound.right  = dX + szx;
	m_rcBound.bottom = dY + szy;
	// === SFML INTERCEPT (Fase 9.B - PutShiftTransSprite2 - Sprite2) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
...
void CSprite::PutFadeSprite(short sX, short sY, short sFrame, DWORD dwTime)
{
 ...
 //int dX,dY,sx,sy,szx,szy,pvx,pvy,sTmp;
	...
	SetRect(&m_rcBound, dX, dY, dX + szx, dY + szy);
	// === SFML INTERCEPT (Fase 9.B - PutFadeSprite - Sprite2) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
...
void CSprite::PutSpriteRGB(int sX, int sY, int sFrame, int sRed, int sGreen, int sBlue, DWORD dwTime)
{
 ...
 WORD * pSrc, * pDst;
	...
	// === SFML INTERCEPT (Fase 9.B - PutSpriteRGB - Sprite2) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
		...
		if (g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			int cx = g_pRenderBackend->GetCropX(), cy = g_pRenderBackend->GetCropY();
			g_pRenderBackend->DrawSpriteColor(dX + cx, dY + cy, sx, sy, szx, szy,
				m_iSpriteIndex, sRed, sGreen, sBlue, 255);
			m_bOnCriticalSection = FALSE;
			return;
		}
	}
...
void CSprite::PutTransSpriteRGB(int sX, int sY, int sFrame, int sRed, int sGreen, int sBlue, DWORD dwTime)
{
 ...
 WORD  * pSrc, * pDst;
	...
	m_rcBound.right  = dX + szx;
	m_rcBound.bottom = dY + szy;
	// === SFML INTERCEPT (Fase 9.B - PutTransSpriteRGB - Sprite2) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
		...
		if (g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			int cx = g_pRenderBackend->GetCropX(), cy = g_pRenderBackend->GetCropY();
			g_pRenderBackend->DrawSpriteColor(dX + cx, dY + cy, sx, sy, szx, szy,
				m_iSpriteIndex, sRed, sGreen, sBlue, 128);
			m_bOnCriticalSection = FALSE;
			return;
		}
	}
...
void CSprite::PutShiftSpriteFast(int sX, int sY, int shX, int shY, int sFrame, DWORD dwTime)
{
 ...
 RECT rcRect;
	...
	m_rcBound.right  = dX + szx;
	m_rcBound.bottom = dY + szy;
	// === SFML INTERCEPT (Fase 9.B - PutShiftSpriteFast - Sprite2) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
...
void CSprite::PutRevTransSprite(int sX, int sY, int sFrame, DWORD dwTime, int alphaDepth)
{
	...
	SetRect(&m_rcBound, dX, dY, dX + szx, dY + szy);
	// === SFML INTERCEPT (Fase 9.B - PutRevTransSprite - Sprite2) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
		...
		if (g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			int cx = g_pRenderBackend->GetCropX(), cy = g_pRenderBackend->GetCropY();
			int sfmlAlpha = (alphaDepth > 12) ? 255 : alphaDepth * 21;
			g_pRenderBackend->DrawSpriteColor(dX + cx, dY + cy, sx, sy, szx, szy,
				m_iSpriteIndex, 255, 255, 255, sfmlAlpha);
			m_bOnCriticalSection = FALSE;
			return;
		}
	}
...
void CSprite::PutSpriteFastEight(int sX, int sY, int sFrame, int sEight, DWORD dwTime)
{
 ...
 RECT rcRect;
	...
	m_rcBound.right  = dX + szx;
	m_rcBound.bottom = dY + szy;
	// === SFML INTERCEPT (Fase 9.B - PutSpriteFastEight - Sprite2) ===
	if (g_pRenderBackend && g_pRenderBackend->IsFrameActive() && m_iSpriteIndex >= 0)
	{
		if (!g_pRenderBackend->IsTextureLoaded(m_iSpriteIndex)) {
			DDSURFACEDESC2 ddsd2; ZeroMemory(&ddsd2, sizeof(ddsd2)); ddsd2.dwSize = sizeof(ddsd2);
